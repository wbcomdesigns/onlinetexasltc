...
/**
	...
	function create_single_vendor_product( $admin_product_id, $vendor_id, $course_ids ) {
		try {
			...
			log_debug( "Created vendor product ID: {$duplicated_product->get_id()} for vendor: {$vendor_id} with closed group: {$group_id} and initial price: {$admin_price}" );
    ...
    // Get admin products for display
