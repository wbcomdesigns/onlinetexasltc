...
function create_single_vendor_product( $admin_product_id, $vendor_id, $course_ids ) {
	try {
		...
		$update_result = wp_update_post( array(
			...
			'post_status' => $vendor_product_status, // Use option value
		...
		log_debug( "Created vendor product ID: {$duplicated_product->get_id()} for vendor: {$vendor_id} with status: {$vendor_product_status}, closed group: {$group_id} and initial price: {$admin_price}" );
...
function is_duplicated( $product_id ){
